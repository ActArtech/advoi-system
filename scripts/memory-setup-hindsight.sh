...
# Hermes persists config under /opt/data inside the container (host: /opt/hermes/data).
HERMES_DATA_IN_CONTAINER="${HERMES_DATA_IN_CONTAINER:-/opt/data}"
...
docker exec "${HERMES_CONTAINER}" mkdir -p "${HERMES_DATA_IN_CONTAINER}/hindsight"
...
printf '%s' "${CONFIG_JSON}" | docker exec -i "${HERMES_CONTAINER}" tee "${HERMES_DATA_IN_CONTAINER}/hindsight/config.json" >/dev/null

# Secrets live in Hermes env file inside container (never echo values)
HERMES_ENV="${HERMES_DATA_IN_CONTAINER}/.env"
docker exec "${HERMES_CONTAINER}" touch "${HERMES_ENV}"

if [[ "${MODE}" == "cloud" ]]; then
  if [[ -z "${HINDSIGHT_API_KEY:-}" ]]; then
    echo "WARN: HINDSIGHT_MODE=cloud but HINDSIGHT_API_KEY not set in shell."
    echo "      Add HINDSIGHT_API_KEY=hs_... to container ${HERMES_ENV} and re-run."
  else
...
# hindsight-embed reads profile env from \$HOME/.hindsight/profiles/<name>.env
# Embedded PostgreSQL (pg0) cannot init as root — run profile + daemon as hermes user.
docker exec -u hermes "${HERMES_CONTAINER}" bash -lc "
  set -euo pipefail
  set -a
  ...
  KEY=\"\${HINDSIGHT_API_LLM_API_KEY:-\${OPENAI_API_KEY}}\"
  uv tool run hindsight-embed profile create hermes --port 9077 \
  ...
  echo 'OK: hindsight-embed profile hermes has HINDSIGHT_API_LLM_API_KEY (user=hermes)'
"
...
echo "--- Starting Hindsight embedded daemon (profile=hermes, user=hermes) ---"
docker exec -u hermes "${HERMES_CONTAINER}" bash -lc \
  "uv tool run hindsight-embed -p hermes daemon start" || \
  echo "WARN: hindsight daemon did not start — check ${HERMES_DATA_IN_CONTAINER}/.hindsight/profiles/hermes.log inside ${HERMES_CONTAINER}"
